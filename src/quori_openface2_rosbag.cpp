...
namespace openface2_ros
{
  ...
  {
  public:
    OpenFace2Ros(NodeHandle &nh, FaceAnalysis::FaceAnalyserParameters &face_analysis_params, const string &output_file_path)
      : nh_(nh)
      , it_(nh_)
	  ...
	  , output_file_path(output_file_path)
    {
      ...
      pnh.param<string>("image_topic", image_topic_, "/astra_ros/devices/default/color/image_color");
      
      const auto base_path = package::getPath("openface2_ros");

      pnh.param<bool>("publish_viz", publish_viz_, true);
      ...
      image_sub_ = it_.subscribe(image_topic_, 1, &OpenFace2Ros::process_incoming_, this);
    ...
    ~OpenFace2Ros()
    {
		cout << "saving file " << output_file_path << endl;
		...
		for (const auto& entry : action_unit_intensities) {
    ...
    void init_openface_()
    {
		...
		for (const string & au: au_classes) {
			action_unit_intensities[au] = vector<double>{};
		}
    ...
    void process_incoming_(const sensor_msgs::ImageConstPtr &img)
    {
        ...
        cv::Mat croppedrgb = cv_ptr_rgb->image(roi);

        // cv::resize(croppedrgb, croppedrgb, cv::Size(300, 150));
        cv_ptr_rgb->image = croppedrgb;
        ...
        if(img->header.seq % rate_ == 0  && !all_models_active)
        {
		...
		for (size_t model = 0; model < face_models.size(); ++model)
		{
			...
			if (active_models[model])
			{
				...
				// record FAUs
				for (const string &au: au_classes) {
					if (aus.find(au) != aus.end() && aus[au].presence == 1) {
						action_unit_intensities[au].push_back(aus[au].intensity);		
					}
					else {
						action_unit_intensities[au].push_back(0.0);		
					}
				}
				// record timestamps
				...
				cout << std::setprecision(3) << std::stod(formatted_time) << endl;

				// cout << img->header.stamp.sec - offset << ' ' << img->header.stamp.nsec << ' ' << img->header.stamp.toSec() - offset << endl;
				timestamps.push_back(formatted_time);
	...
	map<string, vector<double>> action_unit_intensities;
	set<string> au_classes;
	vector<string> timestamps;
	double offset = 1686000000;
...
int main(int argc, char *argv[])
{
	...
	string rosbag_file_dir = "/media/kevin/HADES/splitted_bags";
	vector<string> subdirs;
	// Iterate through the directory using a directory iterator
	for (const auto& entry : fs::directory_iterator(rosbag_file_dir)) {
	...
	for (const string &dir: subdirs) {
		const string dir_name = dir.substr(dir.find("Participant"));

		cout << dir_name << endl;
		
		if (dir_name != "Participant12") {
			continue;
		}
		...
		for (const auto& entry : fs::directory_iterator(dir)) {
			...
			if (fs::is_regular_file(entry)) {
				// Get the file name and print it
				string rosbag_file = entry.path().string();

				string tmp = entry.path().filename().string();
				if (tmp != "Participant12_10.bag" && tmp != "Participant12_11.bag") {
					continue;
				}
				...
				dir_name = dir_name.substr(11);
				dir_name = dir_name.substr(0, dir_name.size() - 4);

				string output_file_path = "/home/kevin/Documents/quori/src/openface2_ros/processed_data/Participant_" + dir_name;
				fs::create_directory(output_file_path);

				cout << output_file_path << endl;
				
				output_file_path += "/fau.txt";

				// cout << output_file_path << endl;

				OpenFace2Ros openface_(nh,face_analysis_params, output_file_path);

				rosbag::Bag bag;
				bag.open(rosbag_file, rosbag::bagmode::Read);
				std::vector<std::string> topics;
				topics.push_back(std::string("/astra_ros/devices/default/color/image_color"));
				rosbag::View view(bag, rosbag::TopicQuery(topics));

				cout << "processing " + entry.path().filename().string() + " ..." << endl;
				for(rosbag::MessageInstance message: rosbag::View(bag)) {
					const sensor_msgs::Image::ConstPtr s = message.instantiate<sensor_msgs::Image>();
					if (s == NULL) {
						continue;
					}
					
					openface_.process_incoming_(s);
				}
				bag.close();
			}
		}
